defmodule Lyt.Migration.MySQL.V01 do
  @moduledoc false

  use Ecto.Migration

  def up(_opts) do
    create table(:lyt_meta, primary_key: false) do
      add(:key, :string, size: 255, primary_key: true)
      add(:value, :string, size: 255, null: false)
    end

    create table(:lyt_sessions, primary_key: false) do
      add(:id, :string, size: 64, primary_key: true)
      add(:user_id, :string, size: 255)
      add(:hostname, :string, size: 255)
      add(:started_at, :naive_datetime)
      add(:ended_at, :naive_datetime)
      add(:entry, :text)
      add(:exit, :text)
      add(:referrer, :text)
      add(:screen_width, :integer)
      add(:screen_height, :integer)
      add(:browser, :text)
      add(:browser_version, :string, size: 255)
      add(:operating_system, :text)
      add(:operating_system_version, :text)
      add(:utm_medium, :string, size: 255)
      add(:utm_source, :string, size: 255)
      add(:utm_campaign, :string, size: 255)
      add(:utm_content, :string, size: 255)
      add(:utm_term, :string, size: 255)
      add(:metadata, :map)

      timestamps(type: :utc_datetime)
    end

    create table(:lyt_events) do
      add(:name, :string, size: 255)
      add(:hostname, :string, size: 255)
      add(:path, :text)
      add(:query, :text)
      add(:metadata, :map)

      add(:session_id, references(:lyt_sessions, type: :string, on_delete: :delete_all), size: 64)

      timestamps(type: :utc_datetime)
    end

    create(index(:lyt_events, [:session_id]))
    create(index(:lyt_events, [:name]))
    create(index(:lyt_events, [:inserted_at]))
    create(index(:lyt_sessions, [:inserted_at]))
  end

  def down(_opts) do
    drop(table(:lyt_events))
    drop(table(:lyt_sessions))
    drop(table(:lyt_meta))
  end
end
